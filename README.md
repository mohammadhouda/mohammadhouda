<div align="center">

<!-- Header Banner -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:161b22,100:1f6feb&height=220&section=header&text=Mohammad%20Houda&fontSize=42&fontColor=ffffff&fontAlignY=35&desc=Full-Stack%20Engineer%20%7C%20Backend%20Architect%20%7C%20Systems%20Builder&descSize=16&descAlignY=55&descColor=8b949e&animation=fadeIn" width="100%" />

</div>

<br/>

<table>
<tr>
<td width="50%" valign="top">

### 🧑‍💻 About Me

I'm a **backend-focused full-stack engineer** based in **Lebanon**, studying Computer Science at **Arab Open University**. I don't just write code — I architect systems that scale.

- 🏗️ Currently building **[Hope Link](https://github.com/mohammad-houda)** — a humanitarian NGO platform with a full admin panel, audit logging, and full-text search powered by PostgreSQL
- ☁️ **AWS Cloud Practitioner** certified with hands-on Azure deployment experience
- 🔍 Obsessed with query optimization, caching strategies, and building production-ready APIs
- 📬 Reach me at **muhamad.houda@gmail.com**

</td>
<td width="50%" valign="top">

### 🏆 Highlights

```text
🔧 Built a Price Scanner system serving
   thousands of users via Firebase Cloud
   Messaging with real-time notifications

⚡ Optimized SQL queries using EXPLAIN
   ANALYZE and Redis caching layers

🔐 Implemented JWT-based role-based
   access control (RBAC) systems

🧱 Designed multi-tenant architectures
   with Prisma ORM & PostgreSQL

📊 Created admin panels with audit
   logging and full-text search using
   GIN indexes & PG triggers
```

</td>
</tr>
</table>

<br/>

<div align="center">

## ⚙️ Tech Stack

</div>

<div align="center">
<table>
<tr>
<td align="center" width="25%">

**Backend**

<img src="https://skillicons.dev/icons?i=nodejs,express,php,postgres&perline=4" />

Node.js · Express · PHP · PostgreSQL

</td>
<td align="center" width="25%">

**Frontend**

<img src="https://skillicons.dev/icons?i=nextjs,react,ts,tailwind&perline=4" />

Next.js · React · TypeScript · Tailwind

</td>
<td align="center" width="25%">

**DevOps & Cloud**

<img src="https://skillicons.dev/icons?i=aws,azure,docker,redis&perline=4" />

AWS · Azure · Docker · Redis

</td>
<td align="center" width="25%">

**Tools & Data**

<img src="https://skillicons.dev/icons?i=prisma,git,github,vscode&perline=4" />

Prisma · Git · GitHub · VS Code

</td>
</tr>
</table>
</div>

<br/>

<div align="center">

## 🚀 Featured Projects

</div>

<div align="center">
<table>
<tr>
<td width="50%" valign="top">

<h3 align="center">🕊️ Hope Link</h3>

<p align="center"><i>Humanitarian NGO Platform</i></p>

A full-stack platform connecting NGOs with communities in need. Features a comprehensive admin panel with settings management, audit logging, and PostgreSQL full-text search with GIN indexes.

**Stack:** `Next.js` `Express` `Prisma` `PostgreSQL` `Tailwind CSS`

<p align="center">
  <img src="https://img.shields.io/badge/Status-In_Development-1f6feb?style=flat-square" />
</p>

</td>
<td width="50%" valign="top">

<h3 align="center">🛍️ SoukAI</h3>

<p align="center"><i>WhatsApp AI Sales Automation</i></p>

AI-powered sales assistant for Lebanese Instagram & WhatsApp fashion boutiques. Features per-store AI learning loops, DB-first architecture to prevent hallucination, and volume-based subscription tiers.

**Stack:** `Node.js` `Fastify` `PostgreSQL` `pgvector` `Redis` `Turborepo`

<p align="center">
  <img src="https://img.shields.io/badge/Status-In_Development-1f6feb?style=flat-square" />
</p>

</td>
</tr>
</table>
</div>

<br/>

<div align="center">

## 📊 GitHub Stats

<img src="https://github-readme-stats.vercel.app/api?username=mohammadhouda&show_icons=true&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=1f6feb&icon_color=1f6feb&text_color=8b949e&ring_color=1f6feb" height="165" />
<img src="https://github-readme-streak-stats.herokuapp.com?user=mohammadhouda&theme=github-dark-blue&hide_border=true&background=0D1117&ring=1F6FEB&fire=1F6FEB&currStreakLabel=1F6FEB&sideLabels=8B949E&dates=8B949E" height="165" />

</div>

<br/>

<div align="center">

## 🤝 Let's Connect

<a href="mailto:muhamad.houda@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-0d1117?style=for-the-badge&logo=gmail&logoColor=ea4335" />
</a>
&nbsp;
<a href="https://linkedin.com/in/mohammad-houda" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-0d1117?style=for-the-badge&logo=linkedin&logoColor=0a66c2" />
</a>
&nbsp;
<a href="https://github.com/mohammad-houda" target="_blank">
  <img src="https://img.shields.io/badge/GitHub-0d1117?style=for-the-badge&logo=github&logoColor=ffffff" />
</a>

<br/><br/>

<img src="https://komarev.com/ghpvc/?username=mohammad-houda&style=flat-square&color=1f6feb&label=Profile+Views" />

<br/><br/>

```
"I don't just write code — I build systems that scale."
```

</div>

<!-- Footer -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:161b22,100:1f6feb&height=120&section=footer" width="100%" />
